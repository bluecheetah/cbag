...
TEST_CASE("transform_htr()", "[track_info]") {
    ...
    auto intv_list = std::vector<std::array<cbag::offset_t, 2>>(
        {std::array<cbag::offset_t, 2>({tr_w, std::numeric_limits<cbag::offset_t>::max()})});

    auto tr_pitch = tr_w + tr_sp;
    auto tr_off = tr_pitch / 2;
    auto tr_info = c_tinfo(tr_dir, tr_w, tr_sp, tr_off, intv_list);
...
TEST_CASE("find_next_htr", "[track_info]") {
    using data_type = std::tuple<cbag::offset_t, cbag::offset_t, cbag::offset_t, cbag::cnt_t, int,
                                 bool, cbag::htr_t>;
    auto fname = "tests/data/test_layout/grid/find_next_htr.yaml";

    auto [tr_w, tr_sp, coord, ntr, mode, even, expect] =
    ...
    auto tr_pitch = tr_w + tr_sp;
    auto tr_off = tr_pitch / 2;
    auto tr_info = c_tinfo(cbag::orientation_2d::VERTICAL, tr_w, tr_sp, tr_off, intv_list);

    auto ans =
        cbag::layout::find_next_htr(tr_info, coord, ntr, static_cast<cbag::round_mode>(mode), even);
...
TEST_CASE("coord_to_htr", "[track_info]") {
    ...
    auto fname = "tests/data/test_layout/grid/coord_to_htr.yaml";
